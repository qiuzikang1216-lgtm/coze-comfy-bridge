...
async function detectProductProfileFromImages({ img_urls, normalized_request }) {
  ...
  "强制规则：",
  "1. 所有图片默认都是同一商品的多角度图、细节图或同组展示图，你必须综合判断主商品。",
  "2. 只要图片中出现清晰商品主体，就必须输出最可能的具体商品名称，不能空泛，不能只写“商品”“产品”“饰品”“家居用品”这类泛词。",
  "3. main_prod 必须尽量具体，例如：耳环、戒指、项链、手提包、马克杯、抱枕、台灯、收纳盒、手机壳、香薰蜡烛、发夹、围巾、茶杯、相框、毛毯等。",
  "4. prod_cat 是更高层品类，例如：饰品、箱包、家居、杯具、服饰、文具、美妆工具、厨房用品。",
  "5. prod_color 填最主要可见颜色，prod_mat 填最主要可见材质。",
  "6. 如果 accessory_override 非空，优先写入 acc_prod。",
  "7. 如果 ratio_override 非空，优先写入 ratio_rule。",
  "8. 如果 scene_preference 非空，优先写入 use_scene。",
  "9. 如果 main_prod_override 非空，必须直接作为 main_prod 输出。",
  "10. 只有在以下情况同时满足时，main_prod 才允许输出“识别不确定”：图片打不开、图片完全无商品主体、主体被严重遮挡到无法判断。",
  "11. 不允许因为不够百分百确定就输出“识别不确定”；必须先给出最可能的商品判断。",
  "12. 不允许输出 Schema 之外的字段。",
  "",
  "输出要求：",
  "- main_prod：具体商品名，不要泛词",
  "- acc_prod：配件/陪体，没有就空字符串",
  "- is_combo：是否组合商品",
  "- is_multiview：是否多视角/多张同商品图",
  "- prod_cat：高层品类",
  "- prod_color：主色",
  "- prod_mat：主材质",
  "- use_scene：适用场景",
